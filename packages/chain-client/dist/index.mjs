...
var ChainAbstraction = class extends EventEmitter {
...
var RPCManager = class extends EventEmitter {
  ...
  async createWebSocketConnection(chain) {
    ...
    try {
      const ws = new WebSocket3(endpoint.provider.wsUrl);
      ws.on("open", () => {
...
var DEXAggregator = class extends EventEmitter {
  logger;
  chainAbstraction;
...
var PriceOracle = class extends EventEmitter {
  ...
  async fetchCoinMarketCapPrice(source, address, chain) {
    try {
      ...
      const infoResponse = await axios3.get(infoUrl, {
        params: infoParams,
        timeout: source.timeout,
        headers: {
          "Accept": "application/json",
      ...
      const priceResponse = await axios3.get(priceUrl, {
        params: priceParams,
        timeout: source.timeout,
        headers: {
          "Accept": "application/json",
...
var ZeroLatencyOracle = class extends EventEmitter {
  logger;
  config;
  pythWs;
  binanceWs;
  ...
  async initializePythStream() {
    this.pythWs = new WebSocket3(this.config.pyth.endpoint);
    this.pythWs.on("open", () => {
  ...
  subscribePyth(symbol) {
    if (!this.pythWs || this.pythWs.readyState !== WebSocket3.OPEN) return;
  ...
  async initializeBinanceStream() {
    ...
    this.binanceWs = new WebSocket3(wsUrl);
  ...
  async initializeDexScreenerStream() {
    this.dexScreenerWs = new WebSocket3(this.config.dexscreener.endpoint);
  ...
  subscribeDexScreener(tokenAddress) {
    if (!this.dexScreenerWs || this.dexScreenerWs.readyState !== WebSocket3.OPEN) return;
    const subscription = {
      method: "subscribe",
      params: {
        type: "token",
...
var RealTimeGasTracker = class extends EventEmitter {
  logger;
  config;
  bloxrouteWs;
  flashbotsWs;
  ...
  async initializeBloxrouteStream() {
    ...
    this.bloxrouteWs = new WebSocket3(this.config.bloxroute.endpoint, {
      headers: {
  ...
  subscribeToBloxrouteGas() {
    if (!this.bloxrouteWs || this.bloxrouteWs.readyState !== WebSocket3.OPEN) return;
  ...
  async initializeFlashbotsStream() {
    ...
    this.flashbotsWs = new WebSocket3(this.config.flashbots.endpoint, {
      headers: {
  ...
  subscribeToFlashbotsGas() {
    if (!this.flashbotsWs || this.flashbotsWs.readyState !== WebSocket3.OPEN) return;
    const subscription = {
      method: "eth_subscribe",
      params: [
        "gasPrice",
...
var SmartRouteEngine = class extends EventEmitter {
  logger;
...
var DynamicBridgeMonitor = class extends EventEmitter {
  ...
  async getStargateCost(fromChain, toChain) {
    try {
      const response = await axios3.post("https://api.stargateprotocol.com/v1/estimate-fees", {
  ...
  async getLayerZeroCost(fromChain, toChain) {
    try {
      const response = await axios3.get(`https://api.layerzero.network/v1/estimate-fees`, {
        params: {
          fromChainId: this.getChainId(fromChain),
          toChainId: this.getChainId(toChain),
  ...
  async getAcrossCost(fromChain, toChain) {
    try {
      const response = await axios3.get("https://across.to/api/suggested-fees", {
        params: {
  ...
  async getHopCost(fromChain, toChain) {
    try {
      const response = await axios3.get("https://hop.exchange/api/v1/estimate-fees", {
        params: {
  ...
  async getCurrentBlockTime(chain) {
    try {
      const response = await axios3.get(`https://api.etherscan.io/api`, {
        params: {
          module: "stats",
          action: "chainsize",
...
var DynamicCostManager = class extends EventEmitter {
  currentSpend = {
    monthly: 0,
    daily: 0,
    hourly: 0,
...
var EnhancedChainClient = class extends EventEmitter {
  logger;
  config;
  chainAbstraction;
...
export { ChainAbstraction, ConnectionPool, CostTracker, DEXAggregator, EnhancedChainClient, MempoolMonitor, PriceOracle, RPCManager, RealTimeGasTracker, SmartRouteEngine, ZeroLatencyOracle, createChainClient, createEnhancedChainClient, createZeroLatencyConfig };
